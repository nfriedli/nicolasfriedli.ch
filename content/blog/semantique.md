...
HTML propose 6 niveaux d’intertitres, mais je conseille de vous limiter aux niveaux 2 ou 3 dans la majorité des cas.
Une page très structurée de documentation peut naturellement aller plus loin.

Il faut respecter la logique hiérarchique: un `h3` ne doit jamais suivre directement `h1`, par exemple.
...
Le web et les paragraphes simples et courts font très bon ménage, parce que toute page n’est pas du Proust.

Souvent, il existe un espacement entre 2 paragraphes successifs; parfois un retrait de la première ligne.
...
La règle [Opquast 137](https://checklists.opquast.com/fr/assurance-qualite-web/le-libelle-de-chaque-lien-decrit-sa-fonction-ou-la-nature-du-contenu-vers-lequel-il-pointe) est claire:

> Le libellé de chaque lien décrit sa fonction ou la nature du contenu vers lequel il pointe.

Avec des unités de sens de tailles raisonnables et des ancres de lien dans le corps du texte, c’est ce qui donne le plus de valeur aux contenus ciblés.
**Des liens aux intitulés porteurs de sens placés dans le bon contexte sont le meilleur outil sémantique possible.**
...
Les balises `i` et `b` sont sémantiquement beaucoup moins intéressantes.
Elles peuvent très bien êtres abandonnées.
...
Le terme (`dt` ou *description term*) est lié à sa description (`dd` ou *description details*).
...
Il existe une balise `details` intéressante pour créer des éléments qui se déploient sans utiliser de Javascript (ou des extensions bizarres dans votre WordPress).
Elle est construite ainsi:

{{< details summary="Comment utiliser `details`?" name="details">}}
Avec Hugo, j’ai un code pour produire ce genre de contenus.
Avec WordPress, il existe désormais un bloc natif.
Et sinon, c’est toujours la même histoire:
...
{{< details summary="Pourquoi c’est intéressant?" name="details">}}
Parce que, comme pour les listes de définitions, le contenu est bien relié au résumé (`summary`).
Cela donne un bonne valeur sémantique aux contenus.

C’est très utile pour proposer une foire aux questions (FAQ).
{{< /details >}}

{{< details summary="Accordéon facile" name="details">}}
Il y a fort longtemps, nous parlions d’accordéon.
En utilisant `details` avec un attribut `name` identique pour plusieurs entrées, quand on en ouvre une, les autres se referment.
Comme sur cette page.

Mais ce n’est pas une utilisation obligatoire.
Il est aussi possible d’ouvrir des «onglets» par défaut avec `open`.
{{< /details >}}

## Éléments multimédias

Pour donner un maximum de valeur aux **images**, il faut toujours leur attribuer une alternative.
...
Il existe d’autres moyens de mettre des contenus en forme.
...
Certes, la description n’est plus utilisée comme critère de classement par les moteurs de recherche.
...
Si l’on veut faire (potentiellement) mieux, il faut attaquer les «données structurées vraiment structurées».
Il n’est pas certaine qu’elles seront utilisées.
