...
Comme beaucoup de périphériques n’en disposent pas, on envoie des polices (web fonts) à chaque visite, au mépris de tout bon sens.
Un choix aussi restreint que celui proposé ici serait suffisant pour peut-être 90% du web.

Je me suis déjà exprimé sur le sujet des [polices système](/blog/polices-systeme/).
Je sais que je prêche dans le désert, mais vous conseille quand même de les installer pour en disposer en local.

Toutes les polices ci-dessous sont disponibles en téléchargement (non pas seulement comme polices web) chez [Google Fonts](https://fonts.google.com/).
Mais j’essaie de vous proposer des liens plus intéressant pour les découvrir.
...
Archivo est une police d’apparence simple et classique (à première vue).
Elle propose un bel italique, avec notamment un f bien différent de la police droite
...
Asap la particularité de ne pas varier en largeur quelle que soit la graisse utilisée.
...
Fira Sans est une des plus belles polices de ces 15 dernières années.
Elle n’est pas variable, mais disponible en 9 grasses différentes.
...
C’est la police (gratuite) qui remplace le mieux San Francisco d’Apple.
La version 4 propose un italique très (trop?) différent du droit.
...
Merriweather est une des polices à empattement les plus lisibles.
Elle est disponible en version variable: graisse, largeur et taille optique.
...
JetBrains Mono est la police que j’utilise dans mon éditeur de texte.
Elle est parfaitement optimisée pour la lecture du code.
Je ne vois pas de raison de chercher plus loin.
