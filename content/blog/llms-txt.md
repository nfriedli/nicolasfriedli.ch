+++
title = "Fichiers llms.txt inutiles pour les intelligences artificielles (IA)"
description = "Les fichiers llms.txt ne sont ni une norme, ni un standard. Ils ne sont pas utilisés par les LLM ou les moteurs de recherche. Il demandent des investissements mais ne servent à rien."
date = 2025-11-24
+++
...
Il n’est pas rare de lire que `llms.txt` est une nouvelle norme ou un nouveau standard.
C’est simplement faux.
C’est seulement une proposition pour une (éventuelle) standardisation.

Investir du temps pour quelque chose qui pourrait changer complètement n’est pas une bonne idée.
Pour devenir un standard, il faut des arguments, du débat, un consensus.
Autant dire que c’est très mal parti pour le moment.

En 2025, `llms.txt` n’a aucune utilité.
Mieux vaut investir votre énergie ailleurs.
Il est urgent d’attendre.
Avec le temps, vous constaterez que ce «standard» mourra dans l’œuf.
Voici pourquoi.

## Le Markdown n’apporte rien

J’adore écrire en Markdown.
Tous les contenus de ce site sont écrits en Markdown.
C’est un excellent langage de balisage léger.
Il est excellent parce qu’il est simple à utiliser pour des êtres humains.

Mais, dans le cas des [grands modèles de langages](https://fr.wikipedia.org/wiki/Grand_mod%C3%A8le_de_langage) (LLM), ce sont des machines qui interprètent le contenu.
...
Si un site a besoin d’un fichier spécifique pour montrer sa structure et ses contenus, il y a un vrai problème.
Dans ce cas, mieux vaut arrêter les dégâts.
Mieux vaut abandonner l’optimisation du référencement (SEO).
Mieux vaut investir dans l’architecture d’information (qui vaut toujours la peine).
...
Des entreprises [expliquent comme créer un tel fichier](https://fr.semrush.com/blog/llms-txt/) tout en disant dans le même billet:
...
On brasse du vent.
On utilise des ressources pour créer du rien.
Et agences de communication continuent de vous expliquer qu’il faut refaire votre site avec un `llms.txt` pour optimiser votre GEO (*Generative Engine Optimization*) et améliorer votre image.
