...
La sortie de [Speedlify](https://www.speedlify.dev/) en [version 2](https://www.zachleat.com/web/speedlify2/) m’a incité à remettre cette vieille page à jour.

## Qu’est-ce qu’un budget de performance?

La première visée d’un budget de performance est de faciliter le suivi continu (monitoring) de ses pages web.
...
Avec un budget défini dans un fichier JSON (mise en forme compacte):
...
- 300ko maximum pour l’ensemble des transferts
- 10 fichiers transférés au plus

**C’est un budget très strict, qui peut être utilisé pour ce site, mais trop restrictif dans bien des cas.**

Comme la source JSON le montre clairement, il y a 3 catégories de mesures:

- la vitesse pure (timings)
- la taille des ressources (resourceSizes)
- le nombre des ressources (resourceCounts)

Dans chaque catégorie, on peut spécifier une ou plusieurs valeurs.

## Budget pour commencer

Il est possible de créer un budget JSON simple pour attaquer le sujet et effectuer ses premiers tests:
...
L’utilité d’un budget de performance dépasse le seul monitoring.

En premier lieu, le budget ouvre les débats dans l’équipe de développement lors de son établissement.
...
- d’arbitrer les demandes pour éviter les compromis nuisibles
...
Si une agence ne semble pas comprendre de quoi il s’agit, si elle refuse d’entrer en matière sur le sujet ou si elle s’avère incapable de respecter les valeurs souhaitées, mieux vaut envisager un autre partenaire.
...
Celles et ceux qui créent des sites en militant pour l’écologie devraient aussi s’intéresser aux budgets de performance.
